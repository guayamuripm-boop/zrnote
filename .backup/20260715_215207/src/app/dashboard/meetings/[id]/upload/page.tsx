...
export default function UploadAudioPage() {
  ...
  const compressFile = useCallback(async (file: File): Promise<File> => {
    ...
    if (result !== file) {
      // Create new File with compressed blob, preserving original name/extension
  ...
  const addFiles = useCallback(async (newFiles: FileList | null) => {
    ...
    for (const file of audioFiles) {
      ...
      // If still too large after compression, mark as error
    ...
    <div className="max-w-lg mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
          Formatos: MP3, M4A, WAV, OGG, WebM, AAC — Máximo 4MB por archivo (se comprime automáticamente)
        </p>
