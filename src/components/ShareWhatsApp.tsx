...
// Free WhatsApp sharing: opens WhatsApp with the minute summary + a link
// prefilled, so the user sends it with one tap. No paid API involved.
export default function ShareWhatsApp({
  ...
  title: string;
  summary?: string | null;
  actionItems?: { description: string; assignee_name?: string | null }[];
}) {
  const share = () => {
    const url = typeof window !== 'undefined' ? window.location.href : '';
    const tasks = (actionItems || []).slice(0, 6);
    const tasksText = tasks.length
      ? '\n\n✅ *Compromisos:*\n' +
        tasks
          .map((t) => `• ${t.description}${t.assignee_name ? ` — ${t.assignee_name}` : ''}`)
          .join('\n')
      : '';
    const text =
      `*${title}*\n\n` +
