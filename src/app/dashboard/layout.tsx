...
export default async function DashboardLayout({
  ...
  return (
    <div className="min-h-screen gradient-mesh">
      {/* Desktop Nav */}
      <nav className="glass-strong sticky top-0 z-50 border-b border-white/20 hidden sm:block">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <Link href="/dashboard" className="flex items-center gap-2.5">
              ...
              <span className="text-zr-navy font-poppins font-bold text-lg tracking-tight hidden sm:block">
                ZRNote
              </span>
            </Link>

            <div className="flex items-center gap-1">
              <Link href="/dashboard" className="px-4 py-2 rounded-xl text-sm font-medium text-zr-blue-mid/70 hover:text-zr-navy hover:bg-white/60 transition-all font-poppins">
                Inicio
              </Link>
              <Link href="/dashboard/meetings" className="px-4 py-2 rounded-xl text-sm font-medium text-zr-blue-mid/70 hover:text-zr-navy hover:bg-white/60 transition-all font-poppins">
                Reuniones
              </Link>
              <Link href="/dashboard/action-items" className="px-4 py-2 rounded-xl text-sm font-medium text-zr-blue-mid/70 hover:text-zr-navy hover:bg-white/60 transition-all font-poppins">
                Tareas
              </Link>
              ...
              <div className="flex items-center gap-3">
                ...
                <form action="/api/auth/signout" method="post">
                  <button type="submit" className="text-xs text-zr-blue-mid/40 hover:text-rose-500 transition font-poppins">
                    Salir
                  </button>
                </form>
              </div>
      ...
      {/* Mobile Bottom Bar */}
      <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-50 glass-strong border-t border-white/20">
        <div className="flex items-center justify-around h-16 px-2">
          <Link href="/dashboard" className="flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl text-zr-blue-mid/50 hover:text-zr-blue transition-all">
            ...
            <span className="text-[10px] font-medium font-poppins">Inicio</span>
          </Link>
          <Link href="/dashboard/meetings" className="flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl text-zr-blue-mid/50 hover:text-zr-blue transition-all">
            ...
            <span className="text-[10px] font-medium font-poppins">Reuniones</span>
          </Link>
          ...
          <Link href="/dashboard/action-items" className="flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl text-zr-blue-mid/50 hover:text-zr-blue transition-all">
            ...
            <span className="text-[10px] font-medium font-poppins">Tareas</span>
          </Link>
          <Link href="/dashboard/profile" className="flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl text-zr-blue-mid/50 hover:text-zr-blue transition-all">
            <div className="w-6 h-6 gradient-primary rounded-full flex items-center justify-center">
              <span className="text-white text-[8px] font-bold font-poppins">{user?.email?.charAt(0).toUpperCase() || '?'}</span>
            </div>
          </Link>
        </div>
      </nav>
