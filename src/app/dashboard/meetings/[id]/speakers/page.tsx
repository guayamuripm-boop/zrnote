...
export default function SpeakersPage() {
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      <div>
        <Link href={`/dashboard/meetings/${params.id}`} className="inline-flex items-center gap-1 text-sm text-zr-blue-mid/50 hover:text-zr-blue font-poppins transition mb-4">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
          Volver
        </Link>
        <h1 className="text-2xl sm:text-3xl font-bold text-zr-navy font-poppins">Identificar Participantes</h1>
      ...
      {speakers.length > 0 && (
        <div className="glass-strong rounded-2xl p-5 sm:p-6 shadow-elevated space-y-3">
            ...
            <div key={i} className="glass rounded-xl p-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 gradient-primary rounded-full flex items-center justify-center">
                  <span className="text-white text-xs font-bold font-poppins">{s.name.charAt(0)}</span>
                </div>
                <div>
                  <p className="font-medium text-sm text-zr-navy font-poppins">{s.name}</p>
                  <p className="text-xs text-zr-blue-mid/40 font-poppins">{s.label}</p>
                </div>
              </div>
              <button
                onClick={() => setSpeakers(speakers.filter((_, j) => j !== i))}
                className="text-zr-blue-mid/30 hover:text-rose-500 transition"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className="w-full gradient-primary text-white py-3 rounded-xl font-medium hover:shadow-lg hover:shadow-indigo-500/25 transition-all font-poppins"
          >
